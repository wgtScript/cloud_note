...
function openDialogAndloadDataByParentGrid(a) {
    ...
    if (!a.dialog.beforeOpenCheckUrl || beforeOpenCheck(replaceUrlParamValueByBrace(a.dialog.beforeOpenCheckUrl, d))) {
        ...
        var e = $("#" + a.dialog.id),
        f = topJUI.config.appendRefererParam ? appendSourceUrlParam(a.dialog.href) : a.dialog.href,
        g = f;
...
function refreshGrid(a, b, c) {
    var d = $("#" + b);
    if (d[a]("unselectAll"), d[a]("uncheckAll"), "datagrid" == a || "edatagrid" == a) 1 == c && d[a]({
        queryParams: {
            clearQueryParams: ""
        }
    }),
...
function(a) {
    ...
    a.fn.iCombogrid.defaults = {
        ...
        onChange: function(b, c) {
            ...
            e = a("#" + d.id);
            d.autocomplete && (e.combogrid("grid").datagrid("load", {
                q: b
...
var openDialog = function(a) {
    ...
    if ("object" == typeof d) openDialogAndloadDataByParentGrid(b);
        ...
        if (topJUI.config.appendRefererParam && (b.dialog.href = appendSourceUrlParam(b.dialog.href)), b.dialog.href.indexOf("{") != -1) {
            var g = b.dialog.href,
            h = b.grid.unselectedMsg || b.grid.uncheckedMsg,
            ...
            c.href = replaceUrlParamValueByBrace(b.dialog.href, f[0]),
            $("#" + c.id).iDialog("openDialog", c),
            b.dialog.href = g
...
function(a) {
    ...
    a.fn.iDatagrid.defaults = {
        ...
        loadFilter: function(a) {
            ...
            b.rows = a[topJUI.config.datagrid.rows] ? a[topJUI.config.datagrid.rows] : a,
            b
        },
        onLoadSuccess: function() {
...
function(a) {
    a.fn.iEdatagrid = function(b, c) {
        ...
        this.each(function() {
            ...
            b.initCreate && (topJUI.config.appendRefererParam && (b.url = appendSourceUrlParam(b.url)), a(this).edatagrid(b))
        })
    },
...
function(a) {
    a.fn.iForm = function(b, c) {
        ...
        this.each(function() {
            opts = a.fn.iForm.parseOptions(this, b),
            a(this).form(opts),
            a.getJSON(opts.url,
            function(b) {
                if (a("#" + opts.id).form("load", b), "string" == typeof opts.editor || "object" == typeof opts.editor) {
                    if ("string" == typeof opts.editor) {
                        var c = [],
                        d = [];
                        c = opts.editor.replace(/'/g, '"').split(",");
                        for (var e = 0; e < c.length; e++) d.push(strToJson(c[e]))
                    } else d = opts.editor;
                    for (var e = 0; e < d.length; e++) {
                        var f = d[e].type,
                        g = d[e].id,
                        h = d[e].field;
                        "kindeditor" == f ? a("iframe").each(function(a) {
                            this.contentWindow.document.body.innerHTML = html_decode(b[h])
                        }) : UE.getEditor(g).ready(function() {
                            UE.getEditor(g).setContent(b[h])
                        })
                    }
                }
            })
        ...
        onClick: function() {
            ...
            if (arrayContain(d, b.handler)) {
                if ("ajaxForm" == b.handler || "multiAjaxForm" == b.handler) {
                    ...
                    var e = a("#" + b.mbId).menubutton("options"),
                    f = e.grid;
                    e.dialog;
                    if (a("#" + b.dialogId).form("validate")) {
                        if (b.ajaxData = a("#" + b.dialogId).serialize(), "multiAjaxForm" == b.handler) {
                            var g = getCheckedRowsData(f.type, f.id);
                            void 0 == f.param && (f.param = topJUI.config.pkName + ":" + topJUI.config.pkName),
                            b.ajaxData += "&" + convertParamObj2Serialize(param2JsonObj(f.param), g)
                        }
                        var h = doAjax(b);
                        if (h.statusCode == topJUI.config.statusCode.success) {
                            if (a("#" + b.dialogId).dialog("close"), "object" == typeof f) if ("datagrid" == f.type) a("#" + f.id).datagrid("reload");
                            else if ("treegrid" == f.type) {
                                var i = getSelectedRowData(f.type, f.id);
                                null == i ? a("#" + f.id).treegrid("reload") : a("#" + f.id).treegrid("reload", i[f.parentIdField])
                            }
    ...
    a.fn.iTreegrid.defaults = {
        ...
        onLoadSuccess: function() {
            ...
            if (b.expandAll && a("#" + b.id).iTreegrid("expandAll"), b.expandRoot) {
                ...
                c && 1 == b.firstLoad && (b.firstLoad = !1, a("#" + b.id).iTreegrid("expand", c[b.idField]))
            }
